...
import 'package:music_room_app/screens/rooms/room_map_screen.dart';
...
import 'package:music_room_app/screens/admin/room_add_screen.dart';
...
import 'package:music_room_app/screens/admin/customer_detail_screen.dart';
...
import 'package:music_room_app/screens/admin/activity_log_screen.dart';
import 'package:music_room_app/screens/admin/room_map_editor_screen.dart';
...
import 'package:music_room_app/screens/home/intro_screen.dart';
import 'package:music_room_app/screens/account/my_page_screen.dart';
...
final routerProvider = Provider<GoRouter>((ref) {
  ...
  return GoRouter(
    ...
    routes: [
      ...
      GoRoute(
          path: '/room-map',
          builder: (context, state) => const RoomMapScreen()),
      GoRoute(
        path: '/rooms/:id',
        ...
        routes: [
          GoRoute(
            path: 'visit',
            builder: (context, state) =>
                VisitRequestScreen(roomId: state.pathParameters['id']!),
          ),
        ],
      ),
      GoRoute(
      ...
      GoRoute(
          path: '/my-page', builder: (context, state) => const MyPageScreen()),
      GoRoute(
        path: '/admin',
        ...
        routes: [
          ...
          GoRoute(
              path: 'room-add',
              builder: (context, state) => const RoomAddScreen()),
          GoRoute(
              path: 'room-edit/:id',
              builder: (context, state) =>
                  RoomEditScreen(roomId: state.pathParameters['id']!)),
          GoRoute(
              path: 'room-memo/:id',
              builder: (context, state) =>
                  RoomMemoScreen(roomId: state.pathParameters['id']!)),
          GoRoute(
              path: 'contract-history/:id',
              builder: (context, state) =>
                  ContractHistoryScreen(roomId: state.pathParameters['id']!)),
          GoRoute(
          ...
          GoRoute(
              path: 'notice-edit',
              builder: (context, state) =>
                  NoticeEditScreen(notice: state.extra as Notice?)),
          GoRoute(
              path: 'settings',
              builder: (context, state) => const AdminSettingsScreen()),
          GoRoute(
              path: 'room-map-editor',
              builder: (context, state) => const RoomMapEditorScreen()),
          GoRoute(
              path: 'activity-log',
    ...
    redirect: (context, state) async {
      ...
      if (user == null) {
        ...
        if (!isPublic) {
          debugPrint(
              '[Router][Reason] User is null and path is protected. Redirecting to /login');
          return '/login';
        }

        // 비로그인 상태에서 공개 경로는 접근 허용
        debugPrint(
            '[Router][Reason] User is null but path is public. Proceeding to $location');
        return null;
      }
      ...
      try {
        ...
        debugPrint(
            '[Router][Reason] User role: ${userDoc.role}, isFirstLogin: ${userDoc.isFirstLogin}');
        ...
        if (location.startsWith('/admin')) {
          if (userDoc.role != 'admin') {
            debugPrint(
                '[Router][Reason] Access denied to /admin for non-admin user. Redirecting to /');
            return '/';
          }
        ...
        if ((location == '/' || location == '/intro') &&
            userDoc.role == 'admin') {
          debugPrint(
              '[Router][Reason] Admin user on home page. Redirecting to /admin');
          return '/admin';
        }
        ...
        debugPrint(
            '[Router][Reason] Exception caught: $e. Redirecting to /login');
        return '/login';
      }
    },
  );
